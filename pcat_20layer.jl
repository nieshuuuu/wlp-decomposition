...
println("="^104)
@printf("%-9s %5s %8s %8s %8s %9s %9s %9s   %8s %8s %+8s\n",
        "group","shell","n_geom","n_PV","n_gate","truth_HU","HU_PV","HU_gate","GT_lip%","ms_lip%","d_lip%")
for r in rows
    @printf("%-9s %5d %8d %8d %8d %9.1f %9.1f %9.1f   %8.1f %8.1f %+8.1f\n",
            r.group, r.shell, r.n_geom, r.n_pv, r.n_gate, r.truthHU, r.hu_pv, r.hu_gate,
            100r.gt_l, 100r.m_l, 100*(r.m_l - r.gt_l))
end
...
println("\nMMD accuracy over $(length(rows)) (shell, group) regions — geometric PV avoidance only:")
@printf("%-9s %8s %8s %8s %8s %9s\n","material","CCC","R2","RMSE_%","bias_%","GTrange_%")
stats = Dict{Symbol,Any}()
for (s,nm) in ((:w,"Water"),(:l,"Lipid"),(:p,"Protein"))
    g = [r[Symbol("gt_",s)] for r in rows]; m = [r[Symbol("m_",s)] for r in rows]
    stats[s] = (g=g, m=m)
    @printf("%-9s %8.4f %8.4f %8.2f %+8.2f %9.2f\n", nm, ccc(g,m), cor(g,m)^2,
            100sqrt(mean((m.-g).^2)), 100mean(m.-g), 100*(maximum(g)-minimum(g)))
end
huerr_pv   = [r.hu_pv - r.truthHU for r in rows]
huerr_gate = [r.hu_gate - r.truthHU for r in rows if !isnan(r.hu_gate)]
...
for g in ("healthy","diseased")
    ...
    CM.lines!(ax1, sh, [r.hu_pv for r in rr]; color=(COLG[g],0.45), linewidth=2,
              label="$g measured, geometric PV")
    CM.scatterlines!(ax1, sh, [r.hu_gate for r in rr]; color=COLG[g], linewidth=2.6,
                     markersize=8, label="$g measured, clinical gate")
end
CM.axislegend(ax1; position=:rt, framevisible=false, labelsize=10, nbanks=2)
...
for (col,(s,nm,c)) in enumerate(((:w,"Water",CM.RGBf(.23,.46,.69)),
    ...
    lo = min(minimum(g),minimum(m)); hi = max(maximum(g),maximum(m))
    ...
    for grp in ("healthy","diseased")
        ...
        CM.scatter!(ax, 100 .* g[sel], 100 .* m[sel]; color=(c, grp=="healthy" ? 0.95 : 0.5),
            ...
            strokewidth=0.5, strokecolor=:white, label=grp)
    end
    ...
    col==3 && CM.axislegend(ax; position=:rb, framevisible=false, labelsize=11)
end
CM.Label(fig[0,:], "PCAT 20-layer analysis — MMD scored with geometric partial-volume " *
    "avoidance (erode $ERODE vox), clinical FAI scored with the [-190,-30] HU gate";
...
open(joinpath(OUT,"pcat_20layer.csv"),"w") do io
    println(io,"group,shell_mm,n_geom,n_pv,n_gate,truth_HU,HU_pv,HU_gate,gt_water,gt_lipid,gt_protein,ms_water,ms_lipid,ms_protein")
    for r in rows
        println(io,"$(r.group),$(r.shell),$(r.n_geom),$(r.n_pv),$(r.n_gate),$(r.truthHU),$(r.hu_pv),$(r.hu_gate),$(r.gt_w),$(r.gt_l),$(r.gt_p),$(r.m_w),$(r.m_l),$(r.m_p)")
    end
end
