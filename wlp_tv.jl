...
#   * lambda does NOT transfer ("Do not copy this number to another scanner/dose: refit on a
#     phantom"). It is a caller argument here, never hardcoded.
...
decodable voxels. Returns the smoothed (f_l, f_p); f_w follows from the simplex closure.
"""
...
"""Simplex projection, applied ONCE (never inside the sweep)."""
function wlp_simplex(fw, fl, fp)
    g = (max(fw, 0.0), max(fl, 0.0), max(fp, 0.0))
    s = sum(g)
    s < 1e-12 ? (NaN, NaN, NaN) : (g[1] / s, g[2] / s, g[3] / s)
end
